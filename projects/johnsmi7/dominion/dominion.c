...
int smithyCard(int currPlayer, struct gameState *state, int handPos)
{
  //+3 Cards
  for (int i = 0; i < 4; i++)
    {
...
int adventurerCard(int drawntreasure, struct gameState *state, int currentPlayer, int *temphand, int z, int cardDrawn)
{
  ...
  while(drawntreasure<2){
    ...
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == smithy)
      drawntreasure++;
    else{
...
int remodelCard(int currentPlayer, struct gameState *state, int choice1, int choice2, int handPos, int i, int j)
{
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case remodel:
      remodelCard(currentPlayer, state, choice1, choice2, handPos, i, j);
      return 0;
    ...
    case steward:
      stewardCard(currentPlayer, state, choice1, choice2, choice3, handPos);
      return 0;
